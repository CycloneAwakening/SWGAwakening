...
import requiredFiles from './json/required.json' with { type: 'json' };

export function getManifest(fullScan, emuPath, checkFiles) {
    ...
    let config = { login: 'live' };
    if (window.electronAPI.existsSync(configFile)) config = JSON.parse(window.electronAPI.readFileSync(configFile));

    if (fullScan || (emuPath && !window.electronAPI.existsSync(window.electronAPI.joinPath(emuPath, "swgemu.cfg")))) {
    ...
    fetch(server[config.login][0].manifestUrl)
        .then(res => res.json())
        .then(body => {
...
function unionByName(a, b) {
    const lookup = {};
    for (const i of b) lookup[i.name] = true;
    const r = [];
    for (const i of a) if (!lookup[i.name]) r.push(i);
    for (const i of b) r.push(i);
    return r;
}

let forks = [];
let canceling = true;
export function install(swgPath, emuPath, fullScan, callbacks = {}) {
    canceling = false;
    getManifest(fullScan, emuPath, checkFiles);
    ...
    function checkFiles(manifest) {
        ...
        const env = { swgPath, emuPath };
        if (fullScan) env.fullScan = true;
        for (let i = 0; i < 1; i++) {
            window.electronAPI.forkProcess('install.mjs', { env });
        }
        // Listen for fork-started to get forkId
        window.electron.ipcRenderer.once('fork-started', (event, { forkId }) => {
            // Send initial message to fork
            window.electronAPI.sendToFork({ forkId, message: files[fileIndex++] });
            forks.push(forkId);
        });
        // Listen for messages from the fork
        window.electron.ipcRenderer.on('fork-message', (event, { forkId, message }) => {
            installedCallback(forkId, message);
        });
    }
    function installedCallback(forkId, message) {
        if (message.complete !== undefined) {
            completedBytes += message.complete;
            progress(completedBytes, totalBytes);
            if (fileIndex == files.length) {
                forks.splice(forks.indexOf(forkId), 1);
                window.electronAPI.killFork(forkId);
                console.log("killing fork");
            }
            else window.electronAPI.sendToFork({ forkId, message: files[fileIndex++] });
    ...
    function progress(completed, total) {
        if (callbacks.progress && !canceling) callbacks.progress(completed, total);
    }
}

export function cancel() {
    canceling = true;
    for (const forkId of forks) window.electronAPI.killFork(forkId);
    forks = [];
}

if (typeof process !== 'undefined' && process.send) {
    import('adm-zip').then(async ({ default: AdmZip }) => {
